...
constexpr auto kPathTracerShader =
    "recore/passes/pm_pathtracer/pm_pathtracer.comp.glsl";

PhotonMappingPathTracerPass::PhotonMappingPathTracerPass(
    const vulkan::Device& device, const scene::GPUScene& scene)
...
void PhotonMappingPathTracerPass::reloadShaders(
  ...
  mPipelineLayout = makeUnique<vulkan::PipelineLayout>({
      ...
      .pushConstants = {{
          ...
          .size = sizeof(PhotonMappingPathTracerPush),
      }},
  });
...
void PhotonMappingPathTracerPass::resize(uint32_t width, uint32_t height) {
...
void PhotonMappingPathTracerPass::execute(
    const vulkan::CommandBuffer& commandBuffer,
    vulkan::RenderFrame& currentFrame) {
  RECORE_GPU_PROFILE_SCOPE(
      currentFrame, commandBuffer, "PhotonMappingPathTracer::execute");
  ...
  PhotonMappingPathTracerPush p{
      .scene = mScene.getSceneDataDeviceAddress(),
      .hashGrid = *mHashGrid,
      .photons = mPhotonsAddress,
      .rngSeed = rngSeed,
  };
...
void PhotonMappingPathTracerPass::setInput(const Input& input) {
  ...
  resources.images[5][0] = {
      .sampler = mSampler->vkHandle(),
      .imageView = input.gMaterial.getView().vkHandle(),
      .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
  };
