...
#include <recore/passes/photontracer/photontracer.h>
#include <recore/passes/pm_pathtracer/pm_pathtracer.h>
...
class PhotonMappingRenderer : public core::Renderer {
 public:
  PhotonMappingRenderer(const vulkan::Device& device,
  ...
  void render(const vulkan::CommandBuffer& commandBuffer,
...

    {
      ...
      mPhotonTracerPass->execute(commandBuffer, frame);

      mPhotonMappingPathTracerPass->execute(commandBuffer, frame);

      {
        commandBuffer.transitionImageLayout(
            mPhotonMappingPathTracerPass->getOutputImage(),
 ...
 private:
  ...
  void buildPasses() {
    ...
    mPhotonTracerPass = addPass<passes::PhotonTracerPass>(mDevice, *mGPUScene);
    mPhotonMappingPathTracerPass = addPass<passes::PhotonMappingPathTracerPass>(
        mDevice, *mGPUScene);
  ...
  void buildPassDepencencies() {
    ...
    mPhotonMappingPathTracerPass->setInput({
        .gPosition = *gBuffer.position,
        .gNormal = *gBuffer.normal,
        .gAlbedo = *gBuffer.albedo,
        .gEmissive = *gBuffer.emission,
        .gMaterial = *gBuffer.material,
    });

    mPhotonMappingPathTracerPass->setPhotons(
        mPhotonTracerPass->getHashGrid(),
        mPhotonTracerPass->getPhotonsAddress());

    mAccumulatorPass->setInput(mPhotonMappingPathTracerPass->getOutputImage());
  ...
  uPtr<passes::PhotonTracerPass> mPhotonTracerPass;
  uPtr<passes::PhotonMappingPathTracerPass> mPhotonMappingPathTracerPass;
  uPtr<passes::AccumulatorPass> mAccumulatorPass;
  uPtr<passes::ToneMappingPass> mToneMappingPass;

  std::vector<passes::Pass*> mPasses;

  friend class PhotonMappingGUI;
};

class PhotonMappingGUI : public core::GUI {
 public:
  explicit PhotonMappingGUI(const vulkan::Device& device,
 ...
 private:
  void renderMainGUI() {
    ImGui::Begin("PhotonMapping");
    ImGui::Text("Framerate: %.1f FPS", ImGui::GetIO().Framerate);

    // Print camera position
    const auto& camera = mRenderer.getScene().getCamera();
    ...
    if (ImGui::CollapsingHeader("Tone Mapping")) {
      auto& settings = mRenderer.mToneMappingPass->settings();

      ImGui::SliderFloat("Gamma", &settings.gamma, 0.f, 10.f);
      ImGui::SliderFloat("Exposure", &settings.exposure, 0.f, 10.f);
    }

    if (ImGui::CollapsingHeader("Pixel Debug")) {
      for (const auto& report : mDebugMessenger.getShaderPrints()) {
        ImGui::Text("%s\n", report.c_str());
      }
    }
  ...
  const vulkan::Device& mDevice;
  PhotonMappingRenderer& mRenderer;
...
void launchPhotonMapping() {
  ...
  auto renderer = makeUnique<PhotonMappingRenderer>(
      app->getDevice(), appSettings.resolution, std::move(scene));

  auto gui = makeUnique<PhotonMappingGUI>(app->getDevice(),
                                          *renderer,
                                          app->getWindow(),
                                          debugMessenger,
